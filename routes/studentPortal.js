const express = require('express');
const router = express.Router();
const Student = require('../models/Student');
const Class = require('../models/Class');
const Assignment = require('../models/Assignment');
const Timetable = require('../models/Timetable');
const Attendance = require('../models/Attendance');
const Marksheet = require('../models/Marksheet');
const auth = require('../middleware/auth');
const mongoose = require('mongoose');

// Helper: find student by linked userId
async function resolveStudentForUser(user) {
  let student = await Student.findOne({ userId: user._id }).populate({
    path: 'class',
    populate: [
      { path: 'students', select: 'firstName lastName rollNumber' },
      { path: 'subjects.teachers', select: 'firstName lastName' }
    ]
  });
  if (student) return student;

  if (user.username) {
    student = await Student.findOne({ email: user.username }).populate({
      path: 'class',
      populate: [
        { path: 'students', select: 'firstName lastName rollNumber' },
        { path: 'subjects.teachers', select: 'firstName lastName' }
      ]
    });
    if (student) {
      student.userId = user._id;
      try { await student.save(); } catch (err) { console.warn('Link failed', err.message); }
      return student;
    }
  }
  return null;
}

// Get student profile
router.get('/me', auth, async (req, res) => {
  try {
    const student = await resolveStudentForUser(req.user);
    if (!student) return res.status(404).json({ message: 'Student not found' });
    res.json({ student, class: student.class });
  } catch (err) { res.status(500).json({ message: 'Server error' }); }
});

// Marksheets for the student
router.get('/me-marksheets', auth, async (req, res) => {
  try {
    const student = await resolveStudentForUser(req.user);
    if (!student) return res.status(404).json({ message: 'Student not found' });
    const marksheets = await Marksheet.find({ studentId: student._id }).sort({ createdAt: -1 });
    res.json(marksheets);
  } catch (err) {
    console.error('Marksheets error:', err);
    res.status(500).json({ message: 'Server error' });
  }
});

router.get('/me/assignments', auth, async (req, res) => {
  try {
    const student = await resolveStudentForUser(req.user);
    if (!student) return res.status(404).json({ message: 'Student not found' });
    const classId = student.class?._id || student.class;
    const { month } = req.query;
    const q = { classId };
    if (month) {
      let start, end;
      if (month.includes('-')) {
        const [y, m] = month.split('-').map(Number);
        start = new Date(y, m - 1, 1);
        end = new Date(y, m, 0, 23, 59, 59, 999);
      } else {
        const m = Number(month);
        const y = Number(req.query.year) || new Date().getFullYear();
        start = new Date(y, m - 1, 1);
        end = new Date(y, m, 0, 23, 59, 59, 999);
      }
      q.$or = [{ dueDate: { $gte: start, $lte: end } }, { createdAt: { $gte: start, $lte: end } }];
    }
    const assignments = await Assignment.find(q).sort({ createdAt: -1 }).populate('createdBy', 'firstName lastName');
    res.json(assignments);
  } catch (err) { res.status(500).json({ message: 'Server error' }); }
});

router.get('/me/timetable', auth, async (req, res) => {
  try {
    const student = await resolveStudentForUser(req.user);
    if (!student) return res.status(404).json({ message: 'Student not found' });
    const classId = student.class?._id || student.class;
    const { month } = req.query;
    const q = { classId };
    if (month) {
      let start, end;
      if (month.includes('-')) {
        const [y, m] = month.split('-').map(Number);
        start = new Date(y, m - 1, 1);
        end = new Date(y, m, 0, 23, 59, 59, 999);
      } else {
        const m = Number(month);
        const y = Number(req.query.year) || new Date().getFullYear();
        start = new Date(y, m - 1, 1);
        end = new Date(y, m, 0, 23, 59, 59, 999);
      }
      q.$or = [{ date: { $gte: start, $lte: end } }, { createdAt: { $gte: start, $lte: end } }];
    }
    const timetables = await Timetable.find(q).sort({ date: -1, createdAt: -1 }).populate('uploadedBy', 'firstName lastName');
    res.json(timetables);
  } catch (err) { res.status(500).json({ message: 'Server error' }); }
});

router.get('/me/attendance', auth, async (req, res) => {
  try {
    const student = await resolveStudentForUser(req.user);
    if (!student) return res.status(404).json({ message: 'Student not found' });
    const classId = student.class?._id || student.class;
    if (!classId) return res.status(400).json({ message: 'No class' });
    const { startDate, endDate } = req.query;
    const q = { classId };
    if (startDate && endDate) q.date = { $gte: new Date(startDate), $lte: new Date(endDate) };
    const records = await Attendance.find(q);
    const totalDays = records.length;
    const presentDays = records.reduce((acc, rec) => {
      const r = rec.records.find(r => r.studentId.toString() === student._id.toString());
      return acc + (r && r.status === 'present' ? 1 : 0);
    }, 0);
    res.json({ totalDays, presentDays, percentage: totalDays ? (presentDays / totalDays) * 100 : 0 });
  } catch (err) { res.status(500).json({ message: 'Server error' }); }
});

router.get('/me/attendance/report', auth, async (req, res) => {
  try {
    const student = await resolveStudentForUser(req.user);
    if (!student) return res.status(404).json({ message: 'Student not found' });
    const classId = student.class?._id || student.class;
    const { startDate, endDate } = req.query;
    const q = { classId };
    if (startDate && endDate) q.date = { $gte: new Date(startDate), $lte: new Date(endDate) };
    const records = await Attendance.find(q).sort({ date: 1 });
    const entries = records.map(rec => {
      const sr = rec.records.find(r => r.studentId.toString() === student._id.toString());
      return { date: rec.date, status: sr ? sr.status : 'not-marked' };
    });
    const totalDays = entries.length;
    const presentDays = entries.reduce((acc, e) => acc + (e.status === 'present' ? 1 : 0), 0);
    res.json({ totalDays, presentDays, percentage: totalDays ? (presentDays / totalDays) * 100 : 0, entries });
  } catch (err) { res.status(500).json({ message: 'Server error' }); }
});

module.exports = router;
